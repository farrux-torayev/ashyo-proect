"use client";
import { ByIcon, CompareIcon, DataIcon, DeliveryIcon, LikeIcon } from "@/assets/icons";
    ...
    <>
     <div className="containers !mt-[10px] !mb-[100px]">
      <div className="flex gap-[15px]">
        <Link href={"/"} className="flex gap-[15px] text-[15px] text-[#B6BABF]">
      ...
      <div>
        ...
        <div className="flex justify-between">
          ...
          <div className="w-[45%]">
            <p className="pb-[36px] text-[#06172D]">Narxi<span className="text-[32px] pl-[20px] font-bold">{formatNumber(singeProduct.price)}</span><span className="text-[24px] font-bold pl-[20px]">uzs</span>  </p>
            ...
            <div>
              <p className="flex items-center gap-[16px] pt-[43px]"> <DeliveryIcon/> <span>Yetkazib berish O’zbekiston bo’ylab  </span> </p>
              <p className="flex items-center gap-[16px] py-[20px]"> <ByIcon/> <span>Do’kondi o’zidan olib ketishingiz mumkin  </span></p>
              <p className="flex items-center gap-[16px]"> <DataIcon/> <span>Tahminiy yetkazib berish vaqti 1 kundan 3 kungacha......  </span> </p>
            </div>
          </div>
        </div>
        <div className="mt-[80px]">
            <div className="flex items-center gap-[85px] mb-[45px]">
                <strong className="font-bold text-[18px]">Telfon xususiyatlari </strong>
                <strong className="font-bold text-[18px]"> Mijozlarni fikrlari </strong>
            </div>
            <div className="w-[45%]">
            {variations?.options?.map((item:any)=>(
            <div key={singeProduct.id} className="py-[5px] border-b-[2px] text-[#545D6A] text-[18px] border-slate3400 border-dashed flex justify-between">
